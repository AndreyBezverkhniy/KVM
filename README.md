...
Точкой входа программы является обязательная функция main, объявленная ровно один раз в одном из модулей. Аргументы функции main зануляются.
...
Вызов функции порождает новый контекст, для которого родительским контекстом будет глобальный контекст программы. Т.е. функция имеет доступ только к переменным, объявленным в её теле, и к глобальным переменным.

![alt text](img/classes/class.context.png)

### Программа

Все функции в каждом модуле сохраняются в ассоциативный массив, где ключ - сигнатура функции, а значение - тело функции.
